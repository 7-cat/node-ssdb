...
// cast responsed strings to js types
var conversions = {
  int: function(val) {
...
ResponseParser.prototype.parse = function(buf) {
  ...
  var resps = [], resp = [];
  var len = buf.length;
  // pointer to loop over buffer
  var p = 0;
  // the position of the first '\n' in an unit
  var q = 0;
  // always the start of the next response
  var r = 0;
  var d;
...
function Connection(port, host, auth, timeout) {
  this.host = host || '0.0.0.0';
  this.port = port || 8888;
  this.auth = auth;  // default: undefined
...
Connection.prototype.onrecv = function(buf) {
  ...
  responses.forEach(function (response) {
    ...
    // emit signals
    self.emit(util.format('status_%s', status), command, error, data);
...
function Client(options) {
  this.conn = new Connection(options.port, options.host, options.auth, options.timeout);
